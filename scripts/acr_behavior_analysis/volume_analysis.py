...
class TrafficVisualization:
    ...
    def analyze_traffic_volume(self, domain, start_time_str, end_time_str, log_dir=None):
        ...
        print(f"{'='*55}")
        print(f"  MACs Tracked  : {', '.join(self.macs)}")
    ...
    def analyze_milliseconds(self, domain, start_time_str, end_time_str, figure_name, action_names=None, sub_periods=None):
        df = pd.read_csv(self.csv_file)
        df.columns = [c.strip().lower() for c in df.columns]
        ...
        for col in required_cols:
            if col not in df.columns:
                raise ValueError(f"ERROR HAPPENED: Missing column: {col}")
    ...
    def analyze_milliseconds_incoming(self, domain, start_time_str, end_time_str, figure_name, action_names=None, sub_periods=None):
        df = pd.read_csv(self.csv_file)
        df.columns = [c.strip().lower() for c in df.columns]
        ...
        incoming_df = df[df['dst_mac'].isin(self.macs)]
        traffic_per_ms = incoming_df.groupby('millisecond')['packet_size'].sum().reset_index()
        traffic_per_ms = traffic_per_ms.sort_values(by='millisecond').reset_index(drop=True)
        ...
        ax.plot(
            ...
            color='blue',
            linewidth=1,
            label='Incoming Traffic (bytes/ms)'
        )
        ...
        plt.xlabel("Time (HH:MM:SS)", fontsize=14)
        plt.ylabel("Bytes Received per Millisecond", fontsize=24)
        plt.title(f"Traffic Spikes for {domain}", fontsize=26)
        handles, labels = ax.get_legend_handles_labels()
        num_entries = len(handles)
...
class TrafficCDFAnalyzerWithLog:
    ...
    def filter_outgoing(self):
        if not self.tv_macs:
            raise ValueError("tv_macs must be provided")
        self.df = self.df[self.df['src_mac'].isin(self.tv_macs)]

    def filter_incoming(self):
        if not self.tv_macs:
            raise ValueError("tv_macs must be provided")
        self.df = self.df[self.df['dst_mac'].isin(self.tv_macs)]
...
if __name__ == "__main__":
    ...
    all_acr_csv = input_path / f"{domain_name}.csv"
