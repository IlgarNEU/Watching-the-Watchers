...
def filter_against_reference(target_folder, csv_filename, reference_file_path):
    ...
    try:
        ...
        if 'analyzed_domain' not in target_df.columns:
            print(f"✗ Error: 'analyzed_domain' column not found in target file")
            return
        ...
        filtered_df = target_df[~target_df['analyzed_domain'].isin(reference_domains)]
