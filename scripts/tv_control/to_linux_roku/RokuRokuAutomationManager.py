...
experiment_logger.addHandler(
    logging.FileHandler(f"ROKUROKUHIGHLEVEL_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
)
...
PROGRESS_FILE = "roku_experiment_progress.json"
RETRY_WAIT = 60
MAX_RETRIES = 30
...
class AutomateRoku:
    ...
    async def run_scenario(self, current_test_scenario):
        if current_test_scenario == 'IDLE':
            ...
            await open_antenna_custom_2()
            await exit_anything()

        elif current_test_scenario == 'ANTENNA 10.1':
            await open_antenna_custom()
            await exit_anything()
        ...
        for i in range(start_i, len(self.ivd_scenario)):
            ...
            if current_acr_state != last_acr_state:
                ...
                if current_acr_state == 'ON':
                    await toggle_acr(enable=True)
                else:
                    await toggle_acr(enable=False)
                await go_home()
                await power_off()
