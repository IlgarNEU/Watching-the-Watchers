#!/usr/bin/env python3
"""
Vizio ACR Experiment — Rich Terminal UI
========================================
Drop-in replacement for VizioAutomationManager.py.

Usage:
    python TUIVizioExperiment.py
"""
...
def render_header(state: ExperimentState) -> Panel:
    ...
    grid.add_row(
        "[bold bright_white]📺 Vizio ACR Experiment[/]",
...
class VizioExperimentTUI:
    ...
    async def run(self):
        ...
        with Live(layout, console=self.console, refresh_per_second=4, screen=True):
            ...
            # ── Experiment loop (no connect, separate toggle_acr_on/off) ──
