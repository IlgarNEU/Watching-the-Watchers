...
experiment_logger.addHandler(
    logging.FileHandler(f"VIZIOHIGHLEVEL_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
)
...
class AutomateVizio:
    ...
    def load_progress(self):
        if os.path.exists(PROGRESS_FILE):
            with open(PROGRESS_FILE, "r") as f:
                ...
                return data["iteration"], data["scenario_index"], data.get("last_acr_state", "OFF")
        return 0, 0, "OFF"
    ...
    async def run_experiment(self):
        ...
        await power_off()
        it_cnt = 2 * len(self.ivd_scenario)
        for i in range(start_i, it_cnt):
