...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(f"tclroku_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"),
...
async def send_key(key, delay=3):
    async with aiohttp.ClientSession() as session:
...
async def open_antenna_api():
    logger.info("Opening Antenna TV...")
    await send_key("Right")
    await send_key("Down")
    await send_key("Select")
    await asyncio.sleep(15)
    logger.info("Antenna TV opened.")
...
async def open_antenna_custom_2():
    ...
    await asyncio.sleep(5)
    await watch("Antenna Custom", WATCH_TIME)
...
async def open_idle():
    ...
    for _ in range (0, 4):
        ...
        await send_key("Home")
async def open_hdmi():
    await launch_app("tvinput.hdmi1")
